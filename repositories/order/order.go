...
func (repo *OrderRepository) GetByID(ID uint) (ResponseOrder, error) {
	...
	if rowsAffected := repo.db.Table("orders as o").Select("o.id as ID, o.user_id as UserID, o.service_id as ServiceID, s.service_title as ServiceTitle, s.price as Price, o.qty as Qty, o.date as Date, o.address as Address, o.city as City, o.phone as Phone, o.status as Status, o.is_paid as IsPaid, o.url as Url").Joins("inner join services as s.id = o.service_id").Joins("inner join payment_methods as pm on pm.id = o.payment_method_id").Where("id = ?", ID).First(&order).RowsAffected; rowsAffected == 0 {
