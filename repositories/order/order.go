...
func (repo *OrderRepository) SetRefund(ID uint) (FormatOrder, error) {
	...
	if rowsAffected := repo.db.Table("orders").Where("id = ? AND is_paid = ? AND (status = ? OR status = ?)", ID, true, "cancel", "rejected").Update("has_refunded", true).RowsAffected; rowsAffected == 0 {
