...
import (
	...
	SeederReview "final-project/seeders/review"
...
func TestUpdate(t *testing.T) {
	...
	t.Run("positive", func(t *testing.T) {
		...
		repo.Insert(mockReview)
		mockReview.ID = 1
		mockReview.Review = "Bagus"
		res, err := repo.Update(mockReview)
		assert.Nil(t, err)
		assert.Equal(t, "Bagus", res.Review)
	})
}
